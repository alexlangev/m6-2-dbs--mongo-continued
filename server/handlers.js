...
const deleteReservation = async (req, res) => {
  ...
  try {
    await client.connect();
    const db = client.db('seats-db');
    let seat = await db.collection('seats').updateOne(query, { 
      $set: { 
        isBooked: false,
        customer: {
          "fullName":"",
...
const updateCustomer = async (req, res) => {
  ...
  try {
    await client.connect();
    const db = client.db('seats-db');
    const selectedSeat = await db.collection('seats').findOne(query);
...
module.exports = { getSeats, bookSeats, deleteReservation, updateCustomer };
